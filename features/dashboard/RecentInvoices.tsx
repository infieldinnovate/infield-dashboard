  ...
  return (
    <Card className={styles.card} padding="none">
      <div className={styles.header}>
        ...
        <Link href="/invoices" className={styles.viewAll}>View all</Link>
      </div>
      <div className={styles.tableWrapper}>
        <table className={styles.table}>
          ...
          <tbody>
            {recent.map((inv) => (
              <tr key={inv.id} onClick={() => setSelectedItem(inv)} style={{ cursor: 'pointer' }}>
                <td>
                  <span className={styles.invId}>{inv.invoiceNumber}</span>
                  <span className={styles.invDate}>{inv.issueDate}</span>
                </td>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.75rem' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                ...
                <span style={{ color: '#6b7280', fontSize: '0.875rem' }}>Invoice #{selectedItem.invoiceNumber}</span>
              </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '0.75rem' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.15rem' }}>
                <span style={{ color: '#6b7280', fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.04em' }}>Issue Date</span>
                <span style={{ fontSize: '0.9rem' }}>{formatDate(selectedItem.issueDate)}</span>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.15rem' }}>
                <span style={{ color: '#6b7280', fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.04em' }}>Due Date</span>
                <span style={{ fontSize: '0.9rem' }}>{formatDate(selectedItem.dueDate)}</span>
              </div>
            </div>

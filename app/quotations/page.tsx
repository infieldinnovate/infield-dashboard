...
export default function QuotationsPage() {
  ...
  const [quotationList, setQuotationList] = useState<Quotation[]>(initialQuotations);
  ...
  React.useEffect(() => {
    ...
    if (pending) {
      ...
      try {
        ...
        setIsFormOpen(true);
        toast.info(`Creating quotation from template: ${data.templateName || ''}`);
      } catch {
  ...
  const filtered = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      data = data.filter((qt) => qt.id.toLowerCase().includes(q) || qt.customerName.toLowerCase().includes(q));
    }
    if (statusFilter) {
  ...
  const openCreate = () => {
    setEditQuotation(null);
    setForm({ ...emptyForm, validUntil: new Date(Date.now() + 14 * 86400000).toISOString().split('T')[0] });
    setIsFormOpen(true);
  };
  ...
  const handleSave = () => {
    ...
    const qtData: Quotation = {
      ...
      createdAt: editQuotation?.createdAt || new Date().toISOString().split('T')[0],
      notes: form.notes,
    };

    if (editQuotation) {
      setQuotationList((prev) => prev.map((q) => (q.id === editQuotation.id ? qtData : q)));
    } else {
  ...
  const columns = [
    { key: 'id', header: 'Quotation #', render: (row: Quotation) => <span className={styles.qtId}>{row.id.toUpperCase()}</span> },
    { key: 'customerName', header: 'Customer', render: (row: Quotation) => <span className={styles.customerName}>{row.customerName}</span> },
    { key: 'createdAt', header: 'Date', render: (row: Quotation) => <span className={styles.date}>{row.createdAt}</span> },
    { key: 'validUntil', header: 'Valid Until', render: (row: Quotation) => <span className={styles.date}>{row.validUntil}</span> },
    { key: 'total', header: 'Amount', align: 'right' as const, render: (row: Quotation) => <span className={styles.amount}>{formatCurrency(row.total)}</span> },
    { key: 'status', header: 'Status', align: 'center' as const, render: (row: Quotation) => <Badge variant={statusVariant[row.status]} size="sm">{row.status}</Badge> },
    { key: 'actions', header: '', align: 'right' as const, render: (row: Quotation) => (
      <RowActions actions={[
        { icon: 'Eye', label: 'Preview', onClick: (e) => { e.stopPropagation(); setPreviewQuotation(row); } },
        ...
        { icon: 'Copy', label: 'Duplicate', onClick: (e) => { e.stopPropagation(); duplicateQuotation(row); } },
        { icon: 'Trash2', label: 'Delete', variant: 'danger', onClick: (e) => { e.stopPropagation(); setDeleteQuotation(row); } },
      ]} />
    )},
  ];
  ...
  return (
    <div className={styles.page}>
      <PageHeader
        ...
        actions={<Button leftIcon={<Icon name="Plus" size={16} />} onClick={openCreate}>Create Quotation</Button>}
      />
      <div className={styles.toolbar}>
        <div className={styles.filters}>
          <SearchBar placeholder="Search quotations..." value={search} onChange={handleSearch} className={styles.search} />
          <Select options={statusFilterOptions} value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className={styles.filter} />
        </div>
        <Button leftIcon={<Icon name="Plus" size={16} />} onClick={openCreate}>Create Quotation</Button>
      </div>

      <div className={styles.tableCard}>
        <Table columns={columns} data={filtered} keyExtractor={(row) => row.id} onRowClick={(row) => setPreviewQuotation(row)} />
      </div>

      <div className={styles.results}>{filtered.length} quotations found</div>

      {/* Preview Modal */}
      <Modal isOpen={!!previewQuotation} onClose={() => setPreviewQuotation(null)} title="Quotation Preview" size="xl">
        {previewQuotation && (
          <DocumentEngine
            document={previewQuotation}
            actions={{
              onEdit: () => { const q = previewQuotation; setPreviewQuotation(null); openEdit(q); },
              onDuplicate: () => { const q = previewQuotation; setPreviewQuotation(null); duplicateQuotation(q); },
              onDelete: () => { const q = previewQuotation; setPreviewQuotation(null); setDeleteQuotation(q); },
            }}
          />
        )}
      </Modal>
      ...
      <Modal isOpen={isFormOpen} onClose={() => { setIsFormOpen(false); setEditQuotation(null); }} title={editQuotation ? 'Edit Quotation' : 'Create Quotation'} size="xl" footer={
        <>
          <Button variant="ghost" onClick={() => { setIsFormOpen(false); setEditQuotation(null); }}>Cancel</Button>
          <Button onClick={handleSave}>{editQuotation ? 'Save Changes' : 'Create Quotation'}</Button>
        </>
      }>
          ...
          <div className={styles.formBottom}>
            <div className={styles.calcFields}>
              <div className={styles.calcRow}>
                <Select label="Discount Type" options={[{ value: 'percentage', label: 'Percentage (%)' }, { value: 'fixed', label: 'Fixed Amount' }]} value={form.discountType} onChange={(e) => setForm({ ...form, discountType: e.target.value as 'percentage' | 'fixed' })} />
      ...
      <ConfirmDialog
        ...
        message={deleteQuotation ? `Are you sure you want to delete quotation "${deleteQuotation.id.toUpperCase()}"? This action cannot be undone.` : ''}
        confirmLabel="Delete"
        variant="danger"
      />
